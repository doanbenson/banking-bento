...
const toAccountItem = (account) => ({
    PK: keys_1.pk.account(account.accountId),
    SK: keys_1.sk.account(),
    entityType: "ACCOUNT",
    ...
    GSI1PK: keys_1.pk.user(account.userId),
    GSI1SK: "ACCOUNT#" + account.accountId
});
const toTransactionItem = (txn) => ({
    PK: keys_1.pk.transaction(txn.transactionId),
    SK: keys_1.sk.transaction(),
    ...
    GSI1PK: keys_1.pk.user(txn.userId),
    GSI1SK: "TXN#" + txn.date + "#" + txn.transactionId
});
const fromAccountItem = (item) => ({
...
const fromTransactionItem = (item) => ({
