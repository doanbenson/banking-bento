...
const isPlaidDepositPayload = (value: unknown): value is PlaidDepositPayload => {
  ...
  return (
    isNonEmptyString(payload.webhook_id) &&
...
const toDepositReceivedEvent = (payload: PlaidDepositPayload): DepositReceivedEvent => ({
  source: "plaid",
